...
{
/// <summary>
/// View wrapper for the add-player modal.
/// </summary>
internal sealed class AddPlayerModalView : BaseView<AddPlayerModalViewModel>
{
    /// <summary>
    /// The USS root class applied to the modal.
    /// </summary>
    public const string RootUssClassName = "add-player-modal-root";

    /// <summary>
    /// The USS class applied to the modal content.
    /// </summary>
    public const string PanelUssClassName = "add-player-modal";

    /// <summary>
    /// The USS class applied to individual player cards.
    /// </summary>
    public const string CardClassName = "add-player-card";

    /// <summary>
    /// The modifier applied to the guest card.
    /// </summary>
    public const string GuestCardModifierName = "--guest";

    /// <summary>
    /// The element name used for the backdrop dismiss target.
    /// </summary>
    public const string BackdropName = "dismiss-add-player";

    /// <summary>
    /// The element name used for the back button.
    /// </summary>
    public const string BackButtonName = "close-add-player";

    /// <summary>
    /// The USS class applied to the card grid scroll view.
    /// </summary>
    ...
    /// <summary>
    /// Creates the add-player modal view.
    /// </summary>
    public AddPlayerModalView([NotNull] ILoggerFactory loggerFactory, [NotNull] AddPlayerModalViewModel viewModel)
        : base(loggerFactory, viewModel.SourceTemplate, viewModel)
    {
    }

    /// <summary>
    /// Rebuilds the rendered player cards from the supplied view models.
    /// </summary>
    ...
    protected override void Bind(VisualElement host, VisualElement root)
    {
        ...
        RegisterViewAction<PointerDownEvent>(_backdrop, new CloseAddPlayerSelectorViewAction(), TrickleDown.TrickleDown);
        RegisterViewAction<PointerDownEvent>(_backButton, new CloseAddPlayerSelectorViewAction(), TrickleDown.TrickleDown);
        RegisterViewAction<ClickEvent>(_backButton, new CloseAddPlayerSelectorViewAction(), TrickleDown.TrickleDown);
    }
    ...
    private void RefreshCards()
    {
        ...
        foreach(AddPlayerCardViewModel card in ViewModel.Cards)
        {
            TemplateContainer cardInstance = ViewModel.CardTemplate.CloneTree();
            VisualElement cardRoot = cardInstance.Q(className: CardClassName) ?? cardInstance;
            cardRoot.AddStyleSheets(cardInstance);
            cardRoot.dataSource = card;
            cardRoot.userData = card;
            cardRoot.EnableInClassList(GuestCardModifierName, card.IsGuest);
    ...
    {
        if(_isCardsDragging || _pendingCardSelection == null)
        {
            return;
        }

        PublishViewAction(new PlayerAddedViewAction
        {
            SelectedProfileId = _pendingCardSelection.PlayerId,
            PlayerType = _pendingCardSelection.PlayerType
        });
    }
    ...
    private static AddPlayerCardViewModel TryGetCardFromEventTarget(object eventTarget)
    {
        ...
        while(element != null)
        {
            if(element.ClassListContains(CardClassName) && element.userData is AddPlayerCardViewModel cardViewModel)
            {
